use std::{time::{Duration, SystemTime}, thread, cmp::max};
...
use crate::mobile_scraper::{data_processor::{self, DataProcessor}, model::{Message, MobileDetails, MobileList, Identity}, get_pages, parse_details};
...
fn download_details(slink: String, values: Vec<MobileList>,sender: &Sender<Message<MobileDetails>>) {
    for ml in &values{
        let url = format!("//www.mobile.bg/pcgi/mobile.cgi?act=4&adv={}&slink={}", 
            ml.get_id(), slink);
